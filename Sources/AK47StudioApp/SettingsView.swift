      ...
      VStack(alignment: .leading, spacing: 22) {
        ...
        SettingsGroup(
          ...
          HStack(alignment: .top, spacing: 10) {
            ...
            Text(
              studioText(
                "기본 새로고침과 직접 report 진단은 읽기 전용입니다. 키별 F5 조회, 시계 동기화, 선택한 내장 모드, 완성된 84키 RGB와 고정 1프레임 LCD bootstrap은 각각 별도 확인이 필요합니다. 정책 v2는 canonical 복구 뒤 정확히 140프레임 경계 plan만 먼저 허용하고, 제출 화면 육안 일치와 두 번째 USB 전원 제거 복구까지 마친 exact 대상에서만 일반 1…140프레임 editor plan을 엽니다. Feature 작업은 유선 revision 0x0115의 FF13을, LCD는 정확한 IF3/IF2 USB ancestry의 FF13+FF68을 사용하며 실패하면 이후 장치 작업을 격리합니다. 기본값 복원·raw LCD payload·키맵·매크로·펌웨어 실행 경로는 잠겨 있습니다.",
                "Normal refresh and direct report diagnostics are read only. The per-key F5 query, clock sync, one selected onboard mode, a complete 84-key RGB table, and the fixed one-frame LCD bootstrap each require separate confirmation. Policy v2 first admits only an exact 140-frame boundary plan after canonical recovery; general immutable 1...140-frame editor plans open only after submitted-display visual match and a second wired-power-removal recovery on the exact target. Feature operations use FF13 on wired revision 0x0115; LCD uses FF13+FF68 with exact IF3/IF2 USB ancestry. Failure quarantines later device operations. Default restore, raw LCD payload, keymap, macro, and firmware execution paths are locked.",
                language: language
              )
            )
            .font(.callout)
