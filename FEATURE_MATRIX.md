...
| 로컬 영상 불러오기 | 같은 `불러오기…`가 AVFoundation-readable MP4/MOV/M4V를 자동 판별하고, 동일 private snapshot의 음소거 preview·재생/일시정지·단일 dual-handle timeline·playhead·`HH:MM:SS.mmm` 시간코드로 구간을 고름. 초점을 둔 handle/playhead는 방향키로 선택 FPS의 drift 없는 output-sample grid를 이동하며, 예상 프레임·작업량 확인 뒤 최대 140개 source frame을 기존 inline 편집기로 전달. 원본과 네트워크는 건드리지 않고 편집 GIF로만 보존 | 구현 | VFR 원본의 실제 presentation-frame 단위 탐색과 색 공간 진단 | L |
